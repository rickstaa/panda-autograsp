...
class CameraChessboardRegistration:
    ...
    def register(sensor, config):
        ...
        while k < num_transform_avg:
            # input("%i of the %i images taken. Click enter to capture an image" % (k, num_transform_avg))
            ...
            # FIXME: Broken resize function

            resized_data = imresize(small_color_im.data.astype(np.float32), color_image_rescale_factor, 'bilinear')
            big_color_im = ColorImage(resized_data.astype(np.uint8), small_color_im._frame)
            corner_px = big_color_im.find_chessboard(sx=sx, sy=sy)
            ...
            # FIXME: NoneType error not catched
